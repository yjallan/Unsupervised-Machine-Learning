#' clustergram for MNIST, using prcomp, instead of princomp




clustergram_hdm.kmeans <- function(Data, k, ...)
{
  ...
  cluster <- cl$cluster
  centers <- cl$centers %*% prcomp(Data)$rotation[,1]	# 1 number per center
...
clustergram_hdm.plot.matlines <- function(X,Y, k.range, 
                                      x.range, y.range , COL, 
...
clustergram_hdm <- function(Data, k.range = 2:10 , 
                        clustering_hdm.function = clustergram_hdm.kmeans,
                        clustergram_hdm.plot = clustergram_hdm.plot.matlines, 
                        line.width = .004, add.center.points = T)
{
  ...
  n <- dim(Data)[1]
  PCA.1 <- Data %*% prcomp(Data)$rotation[,1]	# first principal component of our data
  ...
  for(k in k.range)
  {
    k.clusters <- clustering_hdm.function(Data, k)
  ...
  clustergram_hdm.plot(X,Y, k.range, 
                   x.range, y.range , COL, 
                   add.center.points , centers.points)
