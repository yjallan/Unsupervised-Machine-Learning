...
sapply(PIDica[,!(names(PIDica) %in% c("diabetes"))], function(x) abs(e1071::kurtosis(x)))

ica_kurtosis <- data.frame(apply(PID_ica$S,2,kurtosis))
ica_kurtosis <- cbind(ica_kurtosis,seq(1:8)) # add a column of nos from 1 to 11
#remember, you want as far as 3 (in absolute value) as possible
#ica_kurtosis[,1] <- abs(ica_kurtosis[,1] - 3)
names(ica_kurtosis) = c("kurtosis","component")
ggplot(ica_kurtosis,aes(x = component, y = kurtosis))+geom_bar(stat="identity")+
  xlab("Components") + 
  ylab("Kurtosis") +
  ggtitle("Variable Importance using ICA")
...
# ggplot...
histtop <- ggplot(PIDica) + geom_density(aes(PIDica$V3))
