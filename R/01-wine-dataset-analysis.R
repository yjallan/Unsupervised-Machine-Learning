...
g11 <- ggplot(wq_data, aes(x=PC, y=Variance))+
  ...
  ggtitle("Screen Plot - for Wine Quality") + 
  theme_bw()
...
ica_kurtosis <- data.frame(apply(wq_ica$S,2,kurtosis))
ica_kurtosis <- cbind(ica_kurtosis,seq(1:11)) # add a column of nos from 1 to 11
...
# ggplot...
histtop <- ggplot(wqica) + geom_density(aes(wqica$V6))
histright <- ggplot(wqica) + geom_density(aes(wqica$V7))+coord_flip()
scatterp <- ggplot(wqica) + geom_point(aes(wqica$V6, wqica$V7))
